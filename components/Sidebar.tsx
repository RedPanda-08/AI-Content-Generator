...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className={`px-4 sm:px-6 py-4 sm:py-5 border-b border-neutral-800/50 flex items-center ${isCollapsed ? "justify-center px-4" : "justify-between"}`}>
          <Link href="/" className={`flex items-center gap-2.5 sm:gap-3 transition-all duration-300 ${isCollapsed ? "w-0 opacity-0 overflow-hidden" : "w-auto opacity-100"}`}>
          ...
          <button onClick={() => setIsCollapsed(!isCollapsed)} className="hidden lg:flex items-center justify-center w-8 sm:w-9 h-8 sm:h-9 rounded-lg sm:rounded-xl hover:bg-neutral-800/50 transition-all text-neutral-500 hover:text-white border border-transparent hover:border-neutral-700/50">
            ...
            return (
              <div key={item.name} className="relative group">
                <Link
                  href={item.href}
                  onClick={() => setIsMobileOpen(false)}
                  className={`flex items-center gap-3 sm:gap-4 px-3 sm:px-4 py-3 sm:py-3.5 rounded-lg sm:rounded-xl transition-all duration-200 ${isCollapsed ? "justify-center px-3" : ""} ${isActive ? "bg-gradient-to-r from-orange-500 to-pink-600 text-white shadow-lg shadow-orange-500/25" : "text-neutral-400 hover:text-white hover:bg-neutral-800/40"}`}
                >
                  <item.icon className={`flex-shrink-0 w-5 h-5`} />
                  {!isCollapsed && <span className="font-medium text-sm sm:text-[15px]">{item.name}</span>}
                  {!isCollapsed && isActive && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-white"></div>}
                </Link>
                {isCollapsed && (
          ...
          {!isCollapsed && !loading && (
            <div className="bg-neutral-900/50 rounded-xl p-3 sm:p-4 border border-neutral-800/50 mb-2">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="p-1 bg-yellow-500/10 rounded-full"><Zap className="w-3 h-3 text-yellow-500 fill-yellow-500" /></div>
              </div>
              ...
              <div className="w-full bg-neutral-800 h-1 rounded-full mt-2 sm:mt-3 overflow-hidden">
                <div className="bg-gradient-to-r from-orange-500 to-pink-600 h-full rounded-full transition-all duration-500" style={{ width: `${Math.min(((credits || 0) / maxCredits) * 100, 100)}%` }}></div>
              </div>
            </div>
          )}

          {/* User Info Card */}
          <div className={`flex items-center gap-2.5 sm:gap-3 px-2.5 sm:px-3 py-2.5 sm:py-3 rounded-lg sm:rounded-xl bg-neutral-900/40 border border-neutral-800/50 transition-all hover:bg-neutral-800/40 ${isCollapsed ? "justify-center px-2" : ""}`}>
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-gradient-to-br from-orange-500 to-pink-600 flex items-center justify-center text-white text-sm font-bold shadow-md flex-shrink-0">
              {loading ? "…" : isGuest ? <User className="w-4 h-4 sm:w-5 sm:h-5" /> : initial}
            </div>
            {!isCollapsed && (
              <div className="overflow-hidden flex-1 min-w-0">
                <p className="text-xs sm:text-sm font-semibold text-white truncate">{loading ? "Loading..." : email}</p>
                {!loading && isGuest && <p className="text-[10px] sm:text-[11px] text-neutral-500 truncate flex items-center gap-1"><span className="inline-block w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse"></span> Guest Session</p>}
                {!loading && !isGuest && <p className="text-[10px] sm:text-[11px] text-green-500 truncate flex items-center gap-1"><span className="inline-block w-1.5 h-1.5 rounded-full bg-green-500"></span> Active</p>}
              </div>
            )}
          </div>
